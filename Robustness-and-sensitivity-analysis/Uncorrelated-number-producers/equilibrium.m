...
[ux,uy] = gumbel_copula(N*S,0); %% gumbel copula 

e  = (1-ux).^(-1/xi_x);                %% pareto marginals
es = (1-uy).^(-1/xi_x);
...
[ux,uy] = gumbel_copula(S,rho_z);   %% gumbel copula

z  = (1-ux).^(-1/xi_z);                %% pareto marginals
zs = (1-uy).^(-1/xi_z);

a  = bsxfun(@times, e, z'); 
as = bsxfun(@times, es, zs'); 
...
nn = geornd(zeta,S,1); %% sample from geometric distribution with parameter probN
...
[ux,uy] = gumbel_copula(S,rho_n);   %% gumbel copula
    
nn = floor(log(1-ux)./log(1-zeta));  %% geometric marginals
ns = floor(log(1-uy)./log(1-zeta));
    
clear ux uy

for i = 1:S,
        
randperms = randperm(N);
...
sigma  = trade_elasticity(aanew, Y, P, omega, yy, parameters , step, impshare); 
