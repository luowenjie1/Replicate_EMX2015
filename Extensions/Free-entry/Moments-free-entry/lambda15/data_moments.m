...
for t=1:T,
    ...
    %sector_sales = nansum(data_t); 
    %sector_sales = repmat(sector_sales,NN,1);

    sharedom       = data_t./sector_sales; %% domestic market shares (previously in the model we reported 
...
% Order moments as per Daniel's spreadsheet
...
fRtopL1_s      = 0.108;  % note:- these numbers transcribed from Daniel's Aug 2013 spreadsheet
fRtopL5_s      = 0.325;                             
...
% commented out lines just below do size distribution of establishments by sales
...
% but we do size distribution of establishments by value-added in the data
    
fRtopY1        = 0.543; % note:- these numbers transcribed from Daniel's Aug 2013 spreadsheet
fRtopY5        = 0.758; 
...
agg_fexporters = 0.252; % note:- these numbers transcribed from Daniel's Aug 2013 spreadsheet
agg_impshare   = 0.322; % we should make sure this is weighted average in data, to make comparable to aggregate import share in model
...
beta_import_export_shares   = NaN; % still need to compute this one

Block6         = [agg_fexporters,agg_impshare,aweight,intraindex,corr_export_domestic_shares,corr_import_domestic_shares,beta_import_export_shares];
...
%savefile = 'saved_data_moments_aug2013.mat';
